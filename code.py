...
: """).strip().lower()
        ...
        if options == 'back':
            if history:
                json_data, start_key, start_key2, addon, addon2, skip = history.pop()
                print(f"{Fore.CYAN}Restored previous selection.{Style.RESET_ALL}")
            else:
                print(f"{Fore.RED}No previous selection to return to.{Style.RESET_ALL}")
            continue

        try:
            match int(options):
                case 1:
                    history.append((json_data, start_key, start_key2, addon, addon2, skip))
                    ...
                    while True:
                        ...
                        if sight_option == 'back':
                            print(f"{Fore.CYAN}Returning to the main menu.{Style.RESET_ALL}")
                            break
